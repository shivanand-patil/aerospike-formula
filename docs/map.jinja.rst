...
We will see a quick setup to configure the ``aerospike`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``aerospike`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``aerospike`` formulas under ``/srv/salt/aerospike/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/aerospike/parameters/dns:domain/
    ln -s dns:domain /srv/salt/aerospike/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/aerospike/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/aerospike/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``aerospike`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``aerospike`` formulas under ``/srv/salt/aerospike/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/aerospike/parameters/roles
...
We create a configuration for the role ``aerospike/server`` in ``/srv/salt/aerospike/parameters/roles/aerospike/server.yaml``:
...
We create another configuration for the role ``aerospike/client`` in ``/srv/salt/aerospike/parameters/roles/aerospike/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles aerospike/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles aerospike/client
...
.. code-block:: console
    ...
    mv /srv/salt/aerospike/parameters/dns:domain/  '/srv/salt/aerospike/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-aerospike-as-json:
      test.nop:
        - name: {{ aerospike | json }}
...
Here is an example based on `template-formula/aerospike/config/file.sls`_:
...
This ``sls`` file expose a ``aerospike`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ aerospike['winner'] }}
...
.. _template-formula/aerospike/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/aerospike/config/file.sls
